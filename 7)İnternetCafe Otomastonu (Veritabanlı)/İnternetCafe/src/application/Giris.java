...
public class Giris{
	...
	public Giris() { baglantý=Baglantý.Baglan(); }
    ...
    void event_ekle(ActionEvent event) {
		...
		try {
        	...
        	alert1.setContentText("Yönetici Olamazsýnýz Sadece Kullanýcý\n\n"+mesaj);
			...
			if (mesaj.length()==0) 
			{
				...
				alert.setTitle("Giriþ Doðru");
				alert.setHeaderText("Tekrikler");
				alert.setContentText("Tebrikler Artýk Üyemizsiniz");
			...
			sorguýfadesý.setString(4, rb_kullanýcý_kayit.getText().trim());
    ...
    void event_üyeol(ActionEvent event) {
		try {
			...
			anchorPane = FXMLLoader.load(getClass().getResource("Kayit.fxml"));
