...
public class kaydol_giris implements Initializable{

	String sql="";
	Connection baglantý=null;
	PreparedStatement sorguýfadesý=null;
	ResultSet getirilen=null;
	public kaydol_giris() { baglantý=sqlbag.Baglan(); }
    ...
    @FXML
    private Pane pane_kayit;

    @FXML
    private Pane pane_giris;
    ...
    void event_ekle(ActionEvent event) {
		sql = "insert into gir(kul_ad,sifre,yetki)  values(?,?,?)";
		...
		try {
	    	...
	    	rb_kullanýcý_kayit.setSelected(false);
	...
	void event_login(ActionEvent event) 
	{
		...
		sql = "select * from gir where kul_ad=? and sifre=? and yetki=?";
			...
			if (rb_kullanýcý.isSelected()) 
			{
			    try 
			    {
						AnchorPane anchorPane=FXMLLoader.load(getClass().getResource("kullanicimenü.fxml"));
