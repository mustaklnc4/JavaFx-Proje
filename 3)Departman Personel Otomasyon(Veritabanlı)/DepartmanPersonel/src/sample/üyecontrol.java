...
public class üyecontrol implements Initializable {
    ...
    void event_üye(ActionEvent event) {
        ...
        if (txt_parola.getText().length()<4||txt_parolatekrar.getText().length()<4)
        {
        ...
        {
            combo_yetki.setStyle("-fx-border-color: turquoise;   -fx-border-width:1;");
        }
        else
        {
            mesaj+="Yetki Alanı Boş Girdiniz\n";
            combo_yetki.setStyle("-fx-border-color: red;   -fx-border-width:1;");
        ...
        if (mesaj.length()==0) {
            ...
            if(sonucOptional.get()==ButtonType.OK)
            {
                try
                {
                    ...
                    lbl_deger.setText("Başarılı Ekleme");
                }
    ...
    void event_güncelle(ActionEvent event) {
        ...
        if (mesaj.length()==0) {
            ...
            Optional<ButtonType> sonucOptional=alert.showAndWait();
            sql="update login set kul_ad=?, sifre=?, yetki=? where id=?";
            try
            {
                sorguifadesi=baglantı.prepareStatement(sql);
                sorguifadesi.setString(1, txt_user.getText().trim());
                ...
                sorguifadesi.setInt(4, seciliid);
                sorguifadesi.executeUpdate();
                lbl_deger.setText("Başarılı Güncellendi");
            }
    ...
    void event_sil(ActionEvent event)
    {
        ...
        {lbl_deger.setText("Silme İptal Edildi");}
        ...
        try {
            ...
            while(getirilen.next())
            {
                veriler.add(new kayit_sinif(getirilen.getInt("id"),getirilen.getString("kul_ad"),
                        getirilen.getString("sifre"),getirilen.getString("yetki")));
            }
